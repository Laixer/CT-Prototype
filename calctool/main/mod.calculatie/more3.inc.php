...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_operation'])){
	...
	if($rs_exist_row['TRUE']){
		...
		if(mysql_num_rows($rs_project_chap_check_result)){
			$rs_add_op_qry = sprintf("INSERT INTO tbl_project_operation (Create_date, Chapter_id, Invoice_id, Operation, Description, Priority) VALUES (NOW(), '%s', '80', '%s', '%s', '%s')", $chapter, $operation, $description, ($rs_prio_op_row['Priority']+1));
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_upd_sa_id'])){
	...
	if(!$error_message){
		$rs_update_salary = sprintf("UPDATE tbl_project_calc_third_salary SET Tax_id='%s', Date=STR_TO_DATE('%s', '%%d-%%m-%%Y'), Amount='%s', Comment='%s' WHERE Project_calc_third_salary_id='%s'", $tax_id, $date, $amount, $comment, $salary_id);
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_cs_id'])){
	...
	if(!$error_message){
		...
		$rs_add_salary = sprintf("INSERT INTO tbl_project_calc_third_salary (Create_date, Hour_id, Project_id, Invoice_id, Operation_id, Tax_id, Date, Price, Amount, Priority, Comment) VALUES (NOW(), NULL, '%s', '80', '%s', '%s', STR_TO_DATE('%s', '%%d-%%m-%%Y' ), '%s', '%s', '%s', '%s')", $project_id, $operation_id, $tax_id, $date, $price, $amount, ($rs_prio_cm_row['Priority']+1), $comment);
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_ca_id'])){
	...
	if(!$error_message){
		...
		$rs_add_material = sprintf("INSERT INTO tbl_project_calc_third_material (Create_date, Materialtype, Unit, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount, DB_chain, Priority) VALUES (NOW(), '%s', '%s', '%s', '80', '%s', '%s', '%s', '%s', 'N', '%s')", $type, $unit, $project_id, $operation_id, $tax_id, $price, $amount, ($rs_prio_cm_row['Priority']+1));
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_ce_id'])){
	...
	if(!$error_message){
		...
		$rs_add_physical = sprintf("INSERT INTO tbl_project_calc_third_physical (Create_date, Materialtype, Unit, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount, DB_chain, Priority) VALUES (NOW(), '%s', '%s', '%s', '80', '%s', '%s', '%s', '%s', 'N', '%s')", $type, $unit, $project_id, $operation_id, $tax_id, $price, $amount, ($rs_prio_cm_row['Priority']+1));
...
<div id="page-bgtop">
	<div id="title">
		<div style="float:right">
			...
			<input style="height: 24px; background: #FFF url('../images/next.png') no-repeat top left; padding-left: 20px; background-size: 16px; background-position-x: 2px; background-position-y: 2px;" onclick="window.location='?p_id=138&r_id=<?php echo $_GET['r_id']; ?>'" type="button" value="Derden" />
    	...
    	<table>
			<tr>
					...
					<?php $i=0; while($rs_project_work_row = mysql_fetch_assoc($rs_project_work_result)){ $i++;
						...
						$rs_project_work_op_qry = sprintf("SELECT * FROM tbl_project_operation WHERE Chapter_id='%s' AND Invoice_id=80 ORDER BY Priority ASC", $rs_project_work_row['Project_chapter_id']);
					...
					<table id="tbl_<?php echo $rs_project_work_row['Project_chapter_id']; ?>" width="100%" border="0">
						...
						<?php while($rs_project_work_op_row = mysql_fetch_assoc($rs_project_work_op_result)){
							...
							$rs_calc_physical_qry = sprintf("SELECT cp.Project_calc_third_physical_id, cp.Materialtype, cp.Unit, cp.Price, cp.Amount, cp.Tax_id, t.Tax FROM tbl_project_calc_third_physical AS cp JOIN tbl_tax AS t ON t.Tax_id=cp.Tax_id JOIN tbl_project AS p ON p.Project_id=cp.Project_id WHERE cp.Project_id='%s' AND cp.Invoice_id='80' AND cp.Operation_id='%s' AND p.User_id='%s' ORDER BY Priority DESC", $project_id, $rs_project_work_op_row['Project_operation_id'], $user_id);
						...
						<tr class="tbl-operation">
							...
							<form name="frm_op_chg_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>" id="frm_op_chg_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>" action="" method="post">
								<td>
									<input name="fld_chg_operation" type="text" id="fld_chg_operation" style="width: 99%; height:15px;" maxlength="50" value="<?php echo $rs_project_work_op_row['Operation']; ?>" />
								...
								<td>
									...
									<input name="fld_chg_description" type="text" id="fld_chg_description" style="width: 94%; height:15px;" maxlength="100" value="<?php echo $rs_project_work_op_row['Description']; ?>" />
								</td>
							</form>
							<td>
						...
						<tr style="display:" class="t_total_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
							...
							<td colspan="3">
								<table width="100%">
									...
									<?php while($rs_calc_salary_row = mysql_fetch_assoc($rs_calc_salary_result)){ ?>
									<tr style="display:" class="t_salary_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										<td align="right"><a href="javascript:void(0);" onclick="document.frm_sa_upd_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>.submit()"><img src="../../images/valid.png" width="16" height="16" title="Opslaan" /></a></td>
										<form name="frm_sa_upd_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" id="frm_sa_upd_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" action="" method="post">
										<td><input style="width:99%" class="date" type="text" name="fld_date" id="fld_date_<?php echo $rs_project_work_op_row['Project_operation_id'].'_'.$rs_calc_salary_row['Project_calc_third_salary_id']; ?>" value="<?php echo $rs_calc_salary_row['Date']; ?>" /></td>
										<td align="center">&euro;&nbsp;<?php echo number_format($rs_calc_salary_row['Price'], 2, ',', '.'); ?></td>
										...
										<td align="right">&euro;&nbsp;<?php echo number_format($rs_calc_salary_row['Price']*$rs_calc_salary_row['Amount'], 2, ',', '.'); ?><input type="hidden" name="fld_upd_sa_id" id="fld_upd_sa_id" value="<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" /></td>
										...
										<td align="right">
											<form action="" method="post" name="frm_sa_up_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" id="frm_sa_up_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>">
												<input type="hidden" name="fld_up_sa_id" id="fld_up_sa_id" value="<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" />
											</form>
											<form action="" method="post" name="frm_sa_down_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" id="frm_sa_down_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>">
												<input type="hidden" name="fld_down_sa_id" id="fld_down_sa_id" value="<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" />
											</form>
											<a href="#" onClick="document.frm_sa_up_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>.submit()"><img src="../../images/up.png" width="16" height="16" alt="Regel omhoog" title="Regel omhoog" /></a>
											<a href="#" onClick="document.frm_sa_down_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>.submit()"><img src="../../images/down.png" width="16" height="16" alt="Regel omlaag" title="Regel omlaag" /></a>
										</td>
										<td align="right">
											<form action="" method="post" name="frm_sa_del_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" id="frm_sa_del_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>">
												<input type="hidden" name="fld_del_sa_id" id="fld_del_sa_id" value="<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>" />
											</form>
											<a href="#" onClick="document.frm_sa_del_<?php echo $rs_calc_salary_row['Project_calc_third_salary_id']; ?>.submit()"><img src="../../images/remove.png" width="16" height="16" alt="Verwijderen" title="Verwijderen" /></a>
										</td>
									</tr>
									<?php } ?>
									...
									<tr style="display:" class="t_salary_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td align="center"><input name="fld_price" id="fld_price" style="width:99%" type="text" /></td>
										<td>&nbsp;</td>
									...
									<tr style="display:" class="t_salary_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Salary_noprofit_80'], 2, ',', '.'); ?></i></b></font></td>
										<td align="right">&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td align="right">&euro;&nbsp;<?php echo number_format($rs_calc_material_row['Amount']*($rs_calc_material_row['Price']+(($rs_calc_material_row['Price']/100)*$rs_profit_row['3_Profit_material_sec'])), 2, ',', '.'); ?></td>
										<td align="center">
											<select name="slt_tax" id="slt_tax" style="width: 99%;">
											<?php
											$rs_tax_result = mysql_query("SELECT * FROM tbl_tax") or die("Error: " . mysql_error());
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Material_noprofit_80'], 2, ',', '.'); ?></i></b></font></td>
										<td align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Material_profit_80'], 2, ',', '.'); ?></i></b></font></td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
									</tr>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td align="right">&euro;&nbsp;<?php echo number_format($rs_calc_physical_row['Amount']*($rs_calc_physical_row['Price']+(($rs_calc_physical_row['Price']/100)*$rs_profit_row['3_Profit_physical_sec'])), 2, ',', '.'); ?></td>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td width="16%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Physical_noprofit_80'], 2, ',', '.'); ?></i></b></font></td>
										<td width="16%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Physical_profit_80'], 2, ',', '.'); ?></i></b></font></td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
									</tr>
