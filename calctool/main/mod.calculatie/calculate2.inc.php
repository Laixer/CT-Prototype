...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_operation'])){
	...
	if($rs_exist_row['TRUE']){
		...
		$rs_project_chap_check_result = mysql_query($rs_project_chap_check_qry);
		if(mysql_num_rows($rs_project_chap_check_result) && (!$rs_project_module7_row)){
			$rs_add_op_qry = sprintf("INSERT INTO tbl_project_operation (Create_date, Chapter_id, Invoice_id, Operation, Description, Priority) VALUES (NOW(), '%s', '20', '%s', '%s', '%s')", $chapter, $operation, $description, ($rs_prio_op_row['Priority']+1));
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_cs_id'])){
	...
	if(!$error_message && (!$rs_project_module7_row)){
		$rs_check_salary = sprintf("SELECT * FROM tbl_project_calc_salary WHERE Project_id='%s' AND Invoice_id=20 AND Operation_id='%s'", $project_id, $operation_id);
		...
		if(!empty($rs_check_row['Project_calc_salary_id'])){
			...
			$rs_add_salary = sprintf("INSERT INTO tbl_project_calc_salary (Create_date, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount) VALUES (NOW(), '%s', '20', '%s', '%s', '%s', '%s')", $project_id, $operation_id, $tax_id, $price, $amount);
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_cu_id'])){
	...
	if(!$error_message && (!$rs_project_module7_row)){
		$rs_check_salary = sprintf("SELECT * FROM tbl_project_calc_sum WHERE Project_id='%s' AND Invoice_id=20 AND Operation_id='%s'", $project_id, $operation_id);
		$rs_check_row = mysql_fetch_assoc(mysql_query($rs_check_salary));
		if(!empty($rs_check_row['Project_calc_sum_id'])){
			...
			$rs_add_sum = sprintf("INSERT INTO tbl_project_calc_sum (Create_date, Project_id, Invoice_id, Operation_id, Tax_id, Unit, Price, Amount) VALUES (NOW(), '%s', '20', '%s', '%s', '%s', '%s', '%s')", $project_id, $operation_id, $tax_id, $unit, $price, $amount);
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_ca_id'])){
	...
	if(!$error_message && (!$rs_project_module7_row)){
		...
		$rs_add_material = sprintf("INSERT INTO tbl_project_calc_material (Create_date, Materialtype, Unit, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount, DB_chain, Priority) VALUES (NOW(), '%s', '%s', '%s', '20', '%s', '%s', '%s', '%s', 'N', '%s')", $type, $unit, $project_id, $operation_id, $tax_id, $price, $amount, ($rs_prio_cm_row['Priority']+1));
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_ce_id'])){
	...
	if(!$error_message && (!$rs_project_module7_row)){
		...
		$rs_add_physical = sprintf("INSERT INTO tbl_project_calc_physical (Create_date, Materialtype, Unit, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount, DB_chain, Priority) VALUES (NOW(), '%s', '%s', '%s', '20', '%s', '%s', '%s', '%s', 'N', '%s')", $type, $unit, $project_id, $operation_id, $tax_id, $price, $amount, ($rs_prio_cm_row['Priority']+1));
...
<div id="page-bgtop">
	<div id="title">
		...
		<span>Calculeren Onderaanneming</span>
							...
							<td colspan="10">
								...
								<?php if(($rs_calc_salary_row || $rs_calc_material_num || $rs_calc_physical_num) || !$rs_calc_sum_row){ ?>
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="15%" align="right">&euro;&nbsp;<?php echo number_format($rs_calc_material_row['Amount']*($rs_calc_material_row['Price']+(($rs_calc_material_row['Price']/100)*$rs_profit_row['2_Profit_material'])), 2, ',', '.'); ?></td>
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="10%">&nbsp;</td>
										<td width="10%">&nbsp;</td>
										<td width="15%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Material_noprofit_20'], 2, ',', '.'); ?></i></b></font></td>
										<td width="15%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Material_profit_20'], 2, ',', '.'); ?></i></b></font></td>
										<td width="6%">&nbsp;</td>
										<td width="4%">&nbsp;</td>
										<td width="2%">&nbsp;</td>
									</tr>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="15%" align="right">&euro;&nbsp;<?php echo number_format($rs_calc_physical_row['Amount']*($rs_calc_physical_row['Price']+(($rs_calc_physical_row['Price']/100)*$rs_profit_row['2_Profit_physical'])), 2, ',', '.'); ?></td>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="10%">&nbsp;</td>
										<td width="10%">&nbsp;</td>
										<td width="15%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Physical_noprofit_20'], 2, ',', '.'); ?></i></b></font></td>
										<td width="15%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Physical_profit_20'], 2, ',', '.'); ?></i></b></font></td>
										<td width="6%">&nbsp;</td>
										<td width="4%">&nbsp;</td>
										<td width="2%">&nbsp;</td>
									</tr>
									...
									<tr style="display:" class="t_sum_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="15%" align="right">&euro;&nbsp;<?php echo number_format($rs_calc_sum_row['Amount']*($rs_calc_sum_row['Price']+(($rs_calc_sum_row['Price']/100)*$rs_profit_row['2_Profit_item'])), 2, ',', '.'); ?></td>
										<td width="6%" align="center">
										<?php if(!$rs_project_module7_row){ ?>
											<select name="slt_tax" id="slt_tax" style="width: 99%;">
											<?php
