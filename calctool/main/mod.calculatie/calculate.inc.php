...
<div id="page-bgtop">
	<div id="title">
		...
		<span>Calculeren Aanneming</span>
				...
				<td colspan="2" class="tbl-head">Hoofdstuk toevoegen</td>
					...
					<table id="tbl_<?php echo $rs_project_work_row['Project_chapter_id']; ?>" width="100%" border="0">
						...
						<?php while($rs_project_work_op_row = mysql_fetch_assoc($rs_project_work_op_result)){
							...
							$rs_calc_physical_qry = sprintf("SELECT cp.Project_calc_physical_id, cp.Materialtype, cp.Unit, cp.Price, cp.Amount, cp.Tax_id, t.Tax FROM tbl_project_calc_physical AS cp JOIN tbl_tax AS t ON t.Tax_id=cp.Tax_id JOIN tbl_project AS p ON p.Project_id=cp.Project_id WHERE cp.Project_id='%s' AND cp.Invoice_id='10' AND cp.Operation_id='%s' AND p.User_id='%s' ORDER BY Priority DESC", $project_id, $rs_project_work_op_row['Project_operation_id'], $user_id);
						...
						<tr style="display:" class="t_total_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
							...
							<td colspan="10">
								...
								<?php if(($rs_calc_salary_row || $rs_calc_material_num || $rs_calc_physical_num) || !$rs_calc_sum_row){ ?>
									...
									<?php while($rs_calc_material_row = mysql_fetch_assoc($rs_calc_material_result)){ $tax = $rs_calc_material_row['Tax_id']; ?>
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="6%" align="center">
                                        <?php
										if(!empty($tax)){ if($tax==10){ echo '0%<input type="hidden" id="slt_tax" name="slt_tax" value="10" />'; }else{ echo '21%<input type="hidden" id="slt_tax" name="slt_tax" value="40" />'; } }else{
										?>
											<select name="slt_tax" id="slt_tax" style="width: 99%;">
											<?php
											$rs_tax_result = mysql_query("SELECT * FROM tbl_tax") or die("Error: " . mysql_error());
									...
									<?php while($rs_calc_physical_row = mysql_fetch_assoc($rs_calc_physical_result)){ $tax = $rs_calc_physical_row['Tax_id']; ?>
