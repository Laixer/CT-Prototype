...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_total_less'])){
	...
	if($tax_id == 40){
		$rs_check_result_qry = sprintf("SELECT (IFNULL(Salary_2_notax_40,0)+IFNULL(Material_2_notax_40,0)+IFNULL(Physical_2_notax_40,0)+IFNULL(Sum_2_notax_40,0)) AS Total_2_notax FROM tvw_result_less WHERE Project_id=%s", $project_id);
		$rs_check_result_row = mysql_fetch_assoc(mysql_query($rs_check_result_qry));
	}else if($tax_id == 20){
		$rs_check_result_qry = sprintf("SELECT (IFNULL(Salary_2_notax_20,0)+IFNULL(Material_2_notax_20,0)+IFNULL(Physical_2_notax_20,0)+IFNULL(Sum_2_notax_20,0)) AS Total_2_notax FROM tvw_result_less WHERE Project_id=%s", $project_id);
		$rs_check_result_row = mysql_fetch_assoc(mysql_query($rs_check_result_qry));
	}else{
		$rs_check_result_qry = sprintf("SELECT (IFNULL(Salary_2_notax_10,0)+IFNULL(Material_2_notax_10,0)+IFNULL(Physical_2_notax_10,0)+IFNULL(Sum_2_notax_10,0)) AS Total_2_notax FROM tvw_result_less WHERE Project_id=%s", $project_id);
		$rs_check_result_row = mysql_fetch_assoc(mysql_query($rs_check_result_qry));
	}
	$rs_check_total_qry = sprintf("SELECT SUM(Amount) AS Total_less FROM tbl_less_total WHERE Project_id=%s AND Tax_id=%s AND Invoice_id=20", $project_id, $tax_id);
	$rs_check_total_row = mysql_fetch_assoc(mysql_query($rs_check_total_qry));
	$total_check = ($rs_check_result_row['Total_2_notax']-$rs_check_total_row['Total_less']);
	...
	if(!$error_message){
		$rs_total_less_qry = sprintf("INSERT INTO tbl_less_total (Create_date, Project_id, Invoice_id, Amount, Tax_id, `Comment`) VALUES (NOW(), '%s', '20', '%s', '%s', '%s')", $project_id, $amount, $tax_id, $comment);
...
# Less total
$rs_less_total_qry = sprintf("SELECT lt.*, t.Tax FROM tbl_less_total AS lt JOIN tbl_tax AS t ON t.Tax_id=lt.Tax_id WHERE lt.Project_id='%s' AND Invoice_id=20 ORDER BY lt.Create_date DESC", $project_id);
...
<div id="page-bgtop">
	<div id="title">
		<div style="float:right">
			<input style="height: 24px; background: #FFF url('../images/next.png') no-repeat top left; padding-left: 20px; background-size: 16px; background-position-x: 2px; background-position-y: 2px;" onclick="window.location='?p_id=139&r_id=<?php echo $_GET['r_id']; ?>'" type="button" value="Hoofdaanneming" />
		...
		<span>Minderwerk Onderaanneming</span>
		...
		<?php } ?>
				...
				<div id="tbl_sepcified">
					...
					<?php $i=0; while($rs_project_work_row = mysql_fetch_assoc($rs_project_work_result)){ $i++;
						...
						$rs_project_work_op_qry = sprintf("SELECT * FROM tbl_project_operation WHERE Chapter_id='%s' AND Invoice_id=20 ORDER BY Priority ASC", $rs_project_work_row['Project_chapter_id']);
					...
					<table id="tbl_<?php echo $rs_project_work_row['Project_chapter_id']; ?>" width="100%" border="0">
						...
						<?php while($rs_project_work_op_row = mysql_fetch_assoc($rs_project_work_op_result)){
							$rs_calc_salary_qry = sprintf("SELECT cs.Project_calc_salary_id, cs.Price, cs.Amount, cs.Tax_id, t.Tax FROM tbl_project_calc_salary AS cs JOIN tbl_project AS p ON p.Project_id=cs.Project_id JOIN tbl_tax AS t ON t.Tax_id=cs.Tax_id WHERE cs.Project_id='%s' AND cs.Invoice_id='20' AND cs.Operation_id='%s' AND p.User_id='%s' LIMIT 1", $project_id, $rs_project_work_op_row['Project_operation_id'], $user_id);
							...
							$rs_calc_physical_qry = sprintf("SELECT cp.Project_calc_physical_id, cp.Materialtype, cp.Unit, cp.Price, cp.Amount, t.Tax FROM tbl_project_calc_physical AS cp JOIN tbl_tax AS t ON t.Tax_id=cp.Tax_id JOIN tbl_project AS p ON p.Project_id=cp.Project_id WHERE cp.Project_id='%s' AND cp.Invoice_id='20' AND cp.Operation_id='%s' AND p.User_id='%s' ORDER BY Priority DESC", $project_id, $rs_project_work_op_row['Project_operation_id'], $user_id);
							...
							<td colspan="10">
								...
								<?php if(($rs_calc_salary_row || $rs_calc_material_num || $rs_calc_physical_num) || !$rs_calc_sum_row){ ?>
									...
									<tr style="display:" class="t_material_<?php echo $rs_calc_material2_row['Project_calc_material_id']; ?>">
										...
										<td width="16%" align="right" style="color: #999999">&euro;&nbsp;<?php echo number_format($rs_calc_material_row['Amount']*($rs_calc_material_row['Price']+(($rs_calc_material_row['Price']/100)*$rs_profit_row['2_Profit_material'])), 2, ',', '.'); ?></td>
										<td width="8%" align="center" style="color:#F00">&euro;&nbsp;-
										<?php
										if($rs_calc_material_row['Price'] == $rs_calc_material2_row['Price']){
											$_total1 = ($rs_calc_material_row['Amount']*($rs_calc_material_row['Price']+(($rs_calc_material_row['Price']/100)*$rs_profit_row['2_Profit_material'])))-($rs_calc_material2_row['Price'] + (($rs_calc_material2_row['Price']/100)*$rs_profit_row['2_Profit_material'])) * $rs_calc_material2_row['Amount'];
											echo number_format($_total1, 2, ',', '.');
											$total1 += $_total1;
										}else{
											$_total1 = ($rs_calc_material_row['Amount']*($rs_calc_material_row['Price']+(($rs_calc_material_row['Price']/100)*$rs_profit_row['2_Profit_material'])))-$rs_calc_material2_row['Price'] * $rs_calc_material2_row['Amount'];
											echo number_format($_total1, 2, ',', '.');
											$total1 += $_total1;
										}
										?></td>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_calc_physical_row['Project_calc_physical_id']; ?>">
										...
										<td width="16%" align="right" style="color: #999999">&euro;&nbsp;<?php echo number_format($rs_calc_physical_row['Amount']*($rs_calc_physical_row['Price']+(($rs_calc_physical_row['Price']/100)*$rs_profit_row['2_Profit_physical'])), 2, ',', '.'); ?></td>
										<td width="8%" align="center" style="color:#F00">&euro;&nbsp;-
										<?php
										if($rs_calc_physical_row['Price'] == $rs_calc_physical2_row['Price']){
											$_total2 = ($rs_calc_physical_row['Amount']*($rs_calc_physical_row['Price']+(($rs_calc_physical_row['Price']/100)*$rs_profit_row['2_Profit_physical'])))-($rs_calc_physical2_row['Price'] + (($rs_calc_physical2_row['Price']/100)*$rs_profit_row['2_Profit_physical'])) * $rs_calc_physical2_row['Amount'];
											echo number_format($_total2, 2, ',', '.');
											$total2 += $_total2;
										}else{
											$_total2 = ($rs_calc_physical_row['Amount']*($rs_calc_physical_row['Price']+(($rs_calc_physical_row['Price']/100)*$rs_profit_row['2_Profit_physical'])))-$rs_calc_physical2_row['Price'] * $rs_calc_physical2_row['Amount'];
											echo number_format($_total2, 2, ',', '.');
											$total2 += $_total2;
										} ?>
										</td>
									...
									<tr style="display:" class="t_sum_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="16%" align="right" style="color: #999999">&euro;&nbsp;<?php echo number_format($rs_calc_sum_row['Amount']*($rs_calc_sum_row['Price']+(($rs_calc_sum_row['Price']/100)*$rs_profit_row['2_Profit_item'])), 2, ',', '.'); ?></td>
										<td width="8%" align="center" style="color:#F00">&euro;&nbsp;-
										<?php
										if($rs_calc_sum_row['Price'] == $rs_calc_sum2_row['Price']){
											echo number_format(($rs_calc_sum_row['Amount']*($rs_calc_sum_row['Price']+(($rs_calc_sum_row['Price']/100)*$rs_profit_row['2_Profit_item'])))-($rs_calc_sum2_row['Price'] + (($rs_calc_sum2_row['Price']/100)*$rs_profit_row['2_Profit_item'])) * $rs_calc_sum2_row['Amount'], 2, ',', '.');
										}else{
											echo number_format(($rs_calc_sum_row['Amount']*($rs_calc_sum_row['Price']+(($rs_calc_sum_row['Price']/100)*$rs_profit_row['2_Profit_item'])))-$rs_calc_sum2_row['Price'] * $rs_calc_sum2_row['Amount'], 2, ',', '.');
										} ?>
										</td>
