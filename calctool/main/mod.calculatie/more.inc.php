...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_operation'])){
	...
	if($rs_exist_row['TRUE']){
		...
		if(mysql_num_rows($rs_project_chap_check_result)){
			$rs_add_op_qry = sprintf("INSERT INTO tbl_project_operation (Create_date, Chapter_id, Invoice_id, Operation, Description, Priority) VALUES (NOW(), '%s', '60', '%s', '%s', '%s')", $chapter, $operation, $description, ($rs_prio_op_row['Priority']+1));
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_cs_id'])){
	...
	if(!$error_message){
		...
		$rs_add_hour = sprintf("INSERT INTO tbl_project_calc_hour (Create_date, Project_id, Operation_id, Tax_id, Date, Amount, More_work, Comment) VALUES (NOW(), '%s', '%s', '%s', '%s', '%s', '2', '%s')", $project_id, $operation_id, $tax_id, $date_unix, $amount, $comment);
		...
		$rs_add_salary = sprintf("INSERT INTO tbl_project_calc_third_salary (Create_date, Hour_id, Project_id, Invoice_id, Operation_id, Tax_id, Date, Price, Amount, Priority, Comment) VALUES (NOW(), '%s', '%s', '60', '%s', '%s', STR_TO_DATE('%s', '%%d-%%m-%%Y' ), '%s', '%s', '%s', '%s')", $rs_last_hour_row['Project_hour_id'], $project_id, $operation_id, $tax_id, $date, $rs_profit_row['Hour_salary_sec'], $amount, ($rs_prio_cm_row['Priority']+1), $comment);
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_ca_id'])){
	...
	if(!$error_message){
		...
		$rs_add_material = sprintf("INSERT INTO tbl_project_calc_third_material (Create_date, Materialtype, Unit, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount, DB_chain, Priority) VALUES (NOW(), '%s', '%s', '%s', '60', '%s', '%s', '%s', '%s', 'N', '%s')", $type, $unit, $project_id, $operation_id, $tax_id, $price, $amount, ($rs_prio_cm_row['Priority']+1));
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_ce_id'])){
	...
	if(!$error_message){
		...
		$rs_add_physical = sprintf("INSERT INTO tbl_project_calc_third_physical (Create_date, Materialtype, Unit, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount, DB_chain, Priority) VALUES (NOW(), '%s', '%s', '%s', '60', '%s', '%s', '%s', '%s', 'N', '%s')", $type, $unit, $project_id, $operation_id, $tax_id, $price, $amount, ($rs_prio_cm_row['Priority']+1));
    	...
    	<table>
			<tr>
					...
					<?php $i=0; while($rs_project_work_row = mysql_fetch_assoc($rs_project_work_result)){ $i++;
						...
						$rs_project_work_op_qry = sprintf("SELECT * FROM tbl_project_operation WHERE Chapter_id='%s' AND Invoice_id=60 ORDER BY Priority ASC", $rs_project_work_row['Project_chapter_id']);
					...
					<table id="tbl_<?php echo $rs_project_work_row['Project_chapter_id']; ?>" width="100%" border="0">
						...
						<?php while($rs_project_work_op_row = mysql_fetch_assoc($rs_project_work_op_result)){
							...
							$rs_calc_physical_qry = sprintf("SELECT cp.Project_calc_third_physical_id, cp.Materialtype, cp.Unit, cp.Price, cp.Amount, cp.Tax_id, t.Tax FROM tbl_project_calc_third_physical AS cp JOIN tbl_tax AS t ON t.Tax_id=cp.Tax_id JOIN tbl_project AS p ON p.Project_id=cp.Project_id WHERE cp.Project_id='%s' AND cp.Invoice_id='60' AND cp.Operation_id='%s' AND p.User_id='%s' ORDER BY Priority DESC", $project_id, $rs_project_work_op_row['Project_operation_id'], $user_id);
							...
							<td colspan="3">
								<table width="100%">
									...
									<tr style="display:" class="t_salary_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td align="center">&euro;<?php echo number_format($rs_profit_row['Hour_salary_sec'], 2, ',', '.'); ?></td>
										<td>&nbsp;</td>
									...
									<tr style="display:" class="t_salary_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Salary_noprofit_60'], 2, ',', '.'); ?></i></b></font></td>
										<td align="right">&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td align="right">&euro;&nbsp;<?php echo number_format($rs_calc_material_row['Amount']*($rs_calc_material_row['Price']+(($rs_calc_material_row['Price']/100)*$rs_profit_row['1_Profit_material_sec'])), 2, ',', '.'); ?></td>
										<td align="center">
											<select name="slt_tax" id="slt_tax" style="width: 99%;">
											<?php
											$rs_tax_result = mysql_query("SELECT * FROM tbl_tax") or die("Error: " . mysql_error());
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Material_noprofit_60'], 2, ',', '.'); ?></i></b></font></td>
										<td align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Material_profit_60'], 2, ',', '.'); ?></i></b></font></td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
									</tr>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td align="right">&euro;&nbsp;<?php echo number_format($rs_calc_physical_row['Amount']*($rs_calc_physical_row['Price']+(($rs_calc_physical_row['Price']/100)*$rs_profit_row['1_Profit_physical_sec'])), 2, ',', '.'); ?></td>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td width="16%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Physical_noprofit_60'], 2, ',', '.'); ?></i></b></font></td>
										<td width="16%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Physical_profit_60'], 2, ',', '.'); ?></i></b></font></td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
										<td>&nbsp;</td>
									</tr>
