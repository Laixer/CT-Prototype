...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_operation'])){
	...
	if($rs_exist_row['TRUE']){
		...
		$rs_project_chap_check_result = mysql_query($rs_project_chap_check_qry);
		if(mysql_num_rows($rs_project_chap_check_result) && (!$rs_project_module7_row)){
			$rs_add_op_qry = sprintf("INSERT INTO tbl_project_operation (Create_date, Chapter_id, Invoice_id, Operation, Description, Priority) VALUES (NOW(), '%s', '10', '%s', '%s', '%s')", $chapter, $operation, $description, ($rs_prio_op_row['Priority']+1));
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_cs_id'])){
	...
	if(!$error_message && (!$rs_project_module7_row)){
		$rs_check_salary = sprintf("SELECT * FROM tbl_project_calc_salary WHERE Project_id='%s' AND Invoice_id=10 AND Operation_id='%s'", $project_id, $operation_id);
		$rs_check_row = mysql_fetch_assoc(mysql_query($rs_check_salary));
		if(!empty($rs_check_row['Project_calc_salary_id'])){
			$rs_update_salary = sprintf("UPDATE tbl_project_calc_salary AS cs INNER JOIN tbl_project AS p ON p.Project_id=cs.Project_id SET cs.Tax_id='%s', cs.Amount='%s' WHERE cs.Project_id='%s' AND p.User_id='%s' AND cs.Project_calc_salary_id='%s'", $tax_id, $amount, $project_id, $user_id, $rs_check_row['Project_calc_salary_id']);
			mysql_query($rs_update_salary) or die("Error: " . mysql_error());
		}else{
			$rs_add_salary = sprintf("INSERT INTO tbl_project_calc_salary (Create_date, Project_id, Invoice_id, Operation_id, Tax_id, Amount) VALUES (NOW(), '%s', '10', '%s', '%s', '%s')", $project_id, $operation_id, $tax_id, $amount);
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_cu_id'])){
	...
	if(!$unit){
		$error_message = "Vul een hoeveelheid in";
	}
	...
	if(!$error_message && (!$rs_project_module7_row)){
		$rs_check_salary = sprintf("SELECT * FROM tbl_project_calc_sum WHERE Project_id='%s' AND Invoice_id=10 AND Operation_id='%s'", $project_id, $operation_id);
		$rs_check_row = mysql_fetch_assoc(mysql_query($rs_check_salary));
		if(!empty($rs_check_row['Project_calc_sum_id'])){
			$rs_update_sum = sprintf("UPDATE tbl_project_calc_sum AS cu INNER JOIN tbl_project AS p ON p.Project_id=cu.Project_id SET cu.Tax_id='%s', cu.Unit='%s', cu.Price='%s', cu.Amount='%s' WHERE cu.Project_id='%s' AND p.User_id='%s' AND cu.Project_calc_sum_id='%s'", $tax_id, $unit, $price, $amount, $project_id, $user_id, $rs_check_row['Project_calc_sum_id']);
			mysql_query($rs_update_sum) or die("Error: " . mysql_error());
		}else{
			$rs_add_sum = sprintf("INSERT INTO tbl_project_calc_sum (Create_date, Project_id, Invoice_id, Operation_id, Tax_id, Unit, Price, Amount) VALUES (NOW(), '%s', '10', '%s', '%s', '%s', '%s', '%s')", $project_id, $operation_id, $tax_id, $unit, $price, $amount);
			mysql_query($rs_add_sum) or die("Error: " . mysql_error());
		}
	}
}
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_ca_id'])){
	...
	if(!$error_message && (!$rs_project_module7_row)){
		...
		$rs_add_material = sprintf("INSERT INTO tbl_project_calc_material (Create_date, Materialtype, Unit, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount, DB_chain, Priority) VALUES (NOW(), '%s', '%s', '%s', '10', '%s', '%s', '%s', '%s', 'N', '%s')", $type, $unit, $project_id, $operation_id, $tax_id, $price, $amount, ($rs_prio_cm_row['Priority']+1));
...
if(($_SERVER['REQUEST_METHOD'] == "POST") && ($_POST['fld_op_ce_id'])){
	...
	if(!$error_message && (!$rs_project_module7_row)){
		...
		$rs_add_physical = sprintf("INSERT INTO tbl_project_calc_physical (Create_date, Materialtype, Unit, Project_id, Invoice_id, Operation_id, Tax_id, Price, Amount, DB_chain, Priority) VALUES (NOW(), '%s', '%s', '%s', '10', '%s', '%s', '%s', '%s', 'N', '%s')", $type, $unit, $project_id, $operation_id, $tax_id, $price, $amount, ($rs_prio_cm_row['Priority']+1));
...
$(document).ready(function(){
	function addChapter(e){
		e.preventDefault();
		$.ajax({
			url: "/ajax/calculate.php?r_id=<?php echo $project_id; ?>",
			type: "post",
			data: $("#frm_chap_add").serialize()
		}).done(function(data){
			if(data==2){
				alert("Er bestaat al een hoofdstuk met deze naam");
			}else{
				var add = $('<table style="display:none" width="100%" border="0"><tbody><tr class="tbl-head"><td width="16">&nbsp;</td><td width="16"><a href="#" onclick="document.frm_chap_chg_343.submit()"><img src="../../images/valid.png" width="16" height="16" alt="Opslaan" title="Opslaan"></a></td><td colspan="9"><div style="float:right"><form action="" method="post" name="frm_chap_up_343" id="frm_chap_up_343"><input type="hidden" name="fld_up_chap_id" id="fld_up_chap_id" value="343"></form><form action="" method="post" name="frm_chap_down_343" id="frm_chap_down_343"><input type="hidden" name="fld_down_chap_id" id="fld_down_chap_id" value="343"></form><a href="#" onclick="document.frm_chap_up_343.submit()"><img src="../../images/up.png" width="16" height="16" alt="Hoofdstuk omhoog" title="Hoofdstuk omhoog"></a><a href="#" onclick="document.frm_chap_down_343.submit()"><img src="../../images/down.png" width="16" height="16" alt="Hoofdstuk omlaag" title="Hoofdstuk omlaag"></a></div><div style="padding-left:2px;"><form action="" method="post" name="frm_chap_chg_343" id="frm_chap_chg_343"><input name="fld_chg_chap" type="text" id="fld_chg_chap" style="width: 96%; height:15px; border: none; background-color:transparent; color:#FFF" value="'+data+'" maxlength="50"><input type="hidden" name="fld_chg_chap_id" id="fld_chg_chap_id" value="343"></form></div></td><td width="16"><form action="" method="post" name="frm_chap_del_343" id="frm_chap_del_343"><input type="hidden" name="fld_del_chap_id" id="fld_del_chap_id" value="343"></form><a href="#" onclick="document.frm_chap_del_343.submit()"><img src="../../images/remove.png" width="16" height="16" alt="Verwijderen" title="Verwijderen"></a></td></tr><tr class="tbl-subhead"><td width="16">&nbsp;</td><td width="16">&nbsp;</td><td colspan="3">Uit te voeren werkzaamheden</td><td colspan="6">Omschrijving werkzaamheden voor op de offerte</td><td width="16">&nbsp;</td></tr><form name="frm_op_add_343" id="frm_op_add_343" action="" method="post"></form><tr><td>&nbsp;</td><td><a href="#" onclick="document.frm_op_add_343.submit()"><img src="../../images/add.png" width="16" height="16" alt="Toevoegen" title="Toevoegen"></a></td><td colspan="3"><input name="fld_operation" type="text" id="fld_operation" style="width: 99%;" maxlength="50"><input type="hidden" name="fld_chapter" id="fld_chapter" value="343"></td><td colspan="7"><input name="fld_description" type="text" id="fld_description" style="width: 100%;" maxlength="100"></td></tr></tbody></table>');
				$('#ruler').after(add);
				$(add).show("blind");
				$('#fld_new_chapter').val('');
			}
	...
	$("#frm_chap_add").submit(function(e){ addChapter(e); });
	$("#btn_chap_add").click(function(e){ addChapter(e); });
});
</script>
<div id="page-bgtop">
	<div id="title">
		...
		<span>Calculeren Aanneming (AJAX)</span>
	...
	<table width="100%" border="0">
			<tr class="tbl-subhead">
				<td colspan="2" class="tbl-head">Hoofdstuk toevoegen</td>
						...
						<tr>
							<td width="16"><a href="javascript:void(0);" id="btn_chap_add"><img src="../../images/add.png" width="16" height="16" alt="Toevoegen" title="Toevoegen" /></a></td>
						...
						<tr style="display:" class="t_total_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
							...
							<td colspan="10">
								...
								<?php if(($rs_calc_salary_row || $rs_calc_material_num || $rs_calc_physical_num) || !$rs_calc_sum_row){ ?>
									<tr>
										<td width="11%"><a href="javascript:void(0);" onClick="checkToggle('.t_salary_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>', this);">[+]</a> Arbeid</td>
										<td width="2%">&nbsp;</td>
									...
									<tr style="display:" class="t_salary_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="20%" align="center">&euro;&nbsp;<?php echo number_format($rs_profit_row['Hour_salary'], 2, ',', '.'); ?></td>
										...
										<td width="10%" align="right">&euro;&nbsp;<?php echo number_format($rs_profit_row['Hour_salary']*$rs_calc_salary_row['Amount'], 2, ',', '.'); ?></td>
										<td width="15%">&nbsp;</td>
										<td width="15%">&nbsp;</td>
									...
									<tr>
										<td width="11%"><a href="javascript:void(0);" onClick="checkToggle('.t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>', this);">[+]</a> Materiaal</td>
										<td width="2%">&nbsp;</td>
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="15%" align="right">&euro;&nbsp;<?php echo number_format($rs_calc_material_row['Amount']*($rs_calc_material_row['Price']+(($rs_calc_material_row['Price']/100)*$rs_profit_row['1_Profit_material'])), 2, ',', '.'); ?></td>
									...
									<tr style="display:" class="t_material_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="10%">&nbsp;</td>
										<td width="10%">&nbsp;</td>
										<td width="15%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Material_noprofit_10'], 2, ',', '.'); ?></i></b></font></td>
										<td width="15%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Material_profit_10'], 2, ',', '.'); ?></i></b></font></td>
										<td width="6%">&nbsp;</td>
										<td width="4%">&nbsp;</td>
										<td width="2%">&nbsp;</td>
									</tr>
									...
									<tr>
										<td width="11%"><a href="javascript:void(0);" onClick="checkToggle('.t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>', this);">[+]</a> Materieel</td>
										<td width="2%">&nbsp;</td>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="15%" align="right">&euro;&nbsp;<?php echo number_format($rs_calc_physical_row['Amount']*($rs_calc_physical_row['Price']+(($rs_calc_physical_row['Price']/100)*$rs_profit_row['1_Profit_physical'])), 2, ',', '.'); ?></td>
									...
									<tr style="display:" class="t_physical_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<td width="10%">&nbsp;</td>
										<td width="10%">&nbsp;</td>
										<td width="15%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Physical_noprofit_10'], 2, ',', '.'); ?></i></b></font></td>
										<td width="15%" align="right"><font color="#313131"><b><i>&euro;&nbsp;<?php echo number_format($rs_total_row['Physical_profit_10'], 2, ',', '.'); ?></i></b></font></td>
										<td width="6%">&nbsp;</td>
										<td width="4%">&nbsp;</td>
										<td width="2%">&nbsp;</td>
									</tr>
									...
									<?php } ?>
									<?php if(0){ if($rs_calc_sum_row || (!$rs_calc_salary_row && !$rs_calc_material_num && !$rs_calc_physical_num && !$rs_calc_sum_row)){ ?>
									<tr>
										<td width="11%"><a href="javascript:void(0);" onClick="checkToggle('.t_sum_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>', this);">[+]</a> Totaalpost</td>
										<td width="2%">&nbsp;</td>
										<td width="20%">&nbsp;</td>
										<td width="5%" bgcolor="#CCCCCC" align="center"><b>Eenheid</b></td>
										<td width="10%" bgcolor="#CCCCCC" align="center"><b>Prijs / eenheid</b></td>
									...
									<tr style="display:" class="t_sum_<?php echo $rs_project_work_op_row['Project_operation_id']; ?>">
										...
										<?php }else{ echo "&euro;&nbsp;" . number_format($rs_calc_sum_row['Price'], 2, ',', '.'); } ?>
										</td>
										<td width="10%" align="center">
										<?php if(!$rs_project_module7_row){ ?>
										...
										<?php }else{ echo "&euro;&nbsp;" . number_format($rs_calc_sum_row['Amount'], 2, ',', '.'); } ?>
										</td>
										<td width="15%" align="right">&euro;&nbsp;<?php echo number_format($rs_calc_sum_row['Price']*$rs_calc_sum_row['Amount'], 2, ',', '.'); ?></td>
										<td width="15%" align="right">&euro;&nbsp;<?php echo number_format($rs_calc_sum_row['Amount']*($rs_calc_sum_row['Price']+(($rs_calc_sum_row['Price']/100)*$rs_profit_row['1_Profit_item'])), 2, ',', '.'); ?></td>
										<td width="6%" align="center">
										<?php if(!$rs_project_module7_row){ ?>
											<select name="slt_tax" id="slt_tax" style="width: 99%;">
											<?php
