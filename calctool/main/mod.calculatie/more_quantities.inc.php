...
# 60
$rs_project_empty_total_60_qry = sprintf("SELECT SUM(Hour_amount) AS Hour_amount, SUM(Hour_cost) AS Hour_cost, SUM(1_Material) AS 1_Material, SUM(1_Physical) AS 1_Physical, SUM(1_Total) AS 1_Total FROM tvw_quantities_more WHERE Project_id='%s' AND Invoice_id=60 LIMIT 1", $rs_project_perm_check_row['Project_id']);
$rs_project_empty_total_60_result = mysql_query($rs_project_empty_total_60_qry) or die("Error: " . mysql_error());
$rs_project_empty_total_60_row = mysql_fetch_assoc($rs_project_empty_total_60_result);
# 70
$rs_project_empty_total_70_qry = sprintf("SELECT SUM(Hour_amount) AS Hour_amount, SUM(Hour_cost2) AS Hour_cost, SUM(2_Material) AS 2_Material, SUM(2_Physical) AS 2_Physical, SUM(2_Total) AS 2_Total FROM tvw_quantities_more WHERE Project_id='%s' AND Invoice_id=70 LIMIT 1", $rs_project_perm_check_row['Project_id']);
$rs_project_empty_total_70_result = mysql_query($rs_project_empty_total_70_qry) or die("Error: " . mysql_error());
$rs_project_empty_total_70_row = mysql_fetch_assoc($rs_project_empty_total_70_result);
# 80
$rs_project_empty_total_80_qry = sprintf("SELECT SUM(Hour_amount) AS Hour_amount, SUM(Hour_cost2) AS Hour_cost, SUM(3_Material) AS 3_Material, SUM(3_Physical) AS 3_Physical, SUM(3_Total) AS 3_Total FROM tvw_quantities_more WHERE Project_id='%s' AND Invoice_id=80 LIMIT 1", $rs_project_perm_check_row['Project_id']);
$rs_project_empty_total_80_result = mysql_query($rs_project_empty_total_80_qry) or die("Error: " . mysql_error());
$rs_project_empty_total_80_row = mysql_fetch_assoc($rs_project_empty_total_80_result);

# Super totals
$rs_project_total_qry = sprintf("SELECT SUM(Hour_amount) AS Total_hour_amount, SUM(Hour_cost) AS Total_hour_cost, SUM(Hour_cost2) AS Total_hour_cost2, (IFNULL(SUM(1_Material),0)+IFNULL(SUM(2_Material),0)+IFNULL(SUM(3_Material),0)) AS Total_material, (IFNULL(SUM(1_Physical),0)+IFNULL(SUM(2_Physical),0)+IFNULL(SUM(3_Physical),0)) AS Total_physical, (IFNULL(SUM(1_Total),0)+IFNULL(SUM(2_Total),0)+IFNULL(SUM(3_Total),0)) AS Overall_total FROM tvw_quantities_more WHERE Project_id='%s' LIMIT 1", $rs_project_perm_check_row['Project_id']);
...
$total_hour += $rs_project_empty_total_60_row['Hour_amount'];
$total_hour_cost += $rs_project_empty_total_60_row['Hour_cost'];
$total_material += $rs_project_empty_total_60_row['1_Material'];
$total_physical += $rs_project_empty_total_60_row['1_Physical'];
$total_post += $rs_project_empty_total_60_row['1_Post'];
$total_total += $rs_project_empty_total_60_row['1_Total'];

$total_hour += $rs_project_empty_total_70_row['Hour_amount'];
$total_hour_cost += $rs_project_empty_total_70_row['Hour_cost'];
$total_material += $rs_project_empty_total_70_row['2_Material'];
$total_physical += $rs_project_empty_total_70_row['2_Physical'];
$total_post += $rs_project_empty_total_70_row['2_Post'];
$total_total += $rs_project_empty_total_70_row['2_Total'];

$total_hour += $rs_project_empty_total_80_row['Hour_amount'];
$total_hour_cost += $rs_project_empty_total_80_row['Hour_cost'];
$total_material += $rs_project_empty_total_80_row['3_Material'];
$total_physical += $rs_project_empty_total_80_row['3_Physical'];
$total_post += $rs_project_empty_total_80_row['3_Post'];
$total_total += $rs_project_empty_total_80_row['3_Total'];
...
<div id="page-bgtop">
	<div id="title">
		...
		<span>Uittrekstaat Meerwerk</span>
		...
		<div id="intern">
			<div id="table">
				<table width="100%" border="0">
					<?php if($rs_project_empty_total_60_row['1_Total']){ ?>
					<tr class="tbl-head">
					...
					while($rs_project_work_inv1_row = mysql_fetch_assoc($rs_project_work_inv1_result)){
						$rs_project_quant_operation_qry = sprintf("SELECT quant.*, o.Operation FROM tvw_quantities_more AS quant JOIN tbl_project_operation AS o ON o.Project_operation_id=quant.Operation_id WHERE quant.Chapter_id='%s' AND quant.Invoice_id=60 AND 1_Total IS NOT NULL ORDER BY o.Priority ASC", $rs_project_work_inv1_row['Project_chapter_id']);
						...
						# Sub total 1
						$rs_operation_subtotal_qry = sprintf("SELECT SUM(Hour_amount) AS Total_hour_amount, SUM(Hour_cost) AS Total_hour_cost, SUM(1_Material) AS Total_material, SUM(1_Physical) AS Total_physical, SUM(1_Total) AS Overall_total FROM tvw_quantities_more WHERE Project_id='%s' AND Invoice_id=60 ORDER BY Project_id LIMIT 1", $rs_project_work_inv1_row['Project_id']);
					...
					<?php } ?>
					<?php if($rs_project_empty_total_70_row['2_Total']){ ?>
					<tr class="tbl-head">
					...
					while($rs_project_work_inv2_row = mysql_fetch_assoc($rs_project_work_inv2_result)){
						$rs_project_quant_operation_qry = sprintf("SELECT quant.*, o.Operation FROM tvw_quantities_more AS quant JOIN tbl_project_operation AS o ON o.Project_operation_id=quant.Operation_id WHERE quant.Chapter_id='%s' AND quant.Invoice_id=70 AND 2_Total IS NOT NULL ORDER BY o.Priority ASC", $rs_project_work_inv2_row['Project_chapter_id']);
						$rs_project_quant_operation_result = mysql_query($rs_project_quant_operation_qry) or die("Error: " . mysql_error());
						$rs_project_quant_operation_num = mysql_num_rows($rs_project_quant_operation_result);
						# Chapter total
					...
					<?php } ?>
					<?php if(false){ if($rs_project_empty_total_80_row['3_Total']){ ?>
					<tr class="tbl-head">
					...
					while($rs_project_work_inv3_row = mysql_fetch_assoc($rs_project_work_inv3_result)){
						$rs_project_quant_operation_qry = sprintf("SELECT quant.*, o.Operation FROM tvw_quantities_more AS quant JOIN tbl_project_operation AS o ON o.Project_operation_id=quant.Operation_id WHERE quant.Chapter_id='%s' AND quant.Invoice_id=80 AND 3_Total IS NOT NULL ORDER BY o.Priority ASC", $rs_project_work_inv3_row['Project_chapter_id']);
						$rs_project_quant_operation_result = mysql_query($rs_project_quant_operation_qry) or die("Error: " . mysql_error());
						$rs_project_quant_operation_num = mysql_num_rows($rs_project_quant_operation_result);
						# Chapter total
						$rs_operation_chaptotal_qry = sprintf("SELECT SUM(Hour_amount) AS Total_hour_amount, SUM(Hour_cost2) AS Total_hour_cost, SUM(3_Material) AS Total_material, SUM(3_Physical) AS Total_physical, SUM(3_Total) AS Overall_total FROM tvw_quantities_more WHERE Chapter_id='%s' AND Invoice_id=80 ORDER BY Chapter_id LIMIT 1", $rs_project_work_inv3_row['Project_chapter_id']);
						$rs_operation_chaptotal_result = mysql_query($rs_operation_chaptotal_qry) or die("Error: " . mysql_error());
						$rs_operation_chaptotal_row = mysql_fetch_assoc($rs_operation_chaptotal_result);
						# Sub total 1
